...
## 2026-07-31 宠物透明像素命中遮罩

- 宠物不再以完整 `286×272` 图片按钮矩形参与 Windows 命中测试。前端在图片加载、状态动作切换或缩放后读取当前图像 alpha，以 10 px 网格生成纵向合并的命中条带，并在可见像素周围只保留一格点击余量；透明像素直接穿透给下层应用。
- 自定义资源若因资源协议限制不能读取像素，则退回到水平内缩 18%、顶部内缩 10% 的中央拖动范围。遮罩对复杂图片逐级增大网格，最多为宠物保留 240 个矩形；Rust 总上限为 256，避免恶意图片制造无界命中数据。
- 4/4 遮罩定向 Vitest、TypeScript、production build、Rust check/fmt 通过。当前源码由隔离 Cargo 目录构建，未关闭用户正在运行的 Debug 实例；随后 `--skip-build` 原生 run `.cache/native-windows-smoke/1785483027630-11528` 完整通过，覆盖真实宠物点击/拖动/爱心、悬停、设置、七状态、透明合成、60 秒 sleeping、托盘、重启与安全清理。

## 2026-07-29 边缘停靠与中英文切换

- 设置新增 `language: "zh-CN" | "en"`，由现有 Rust Store 规范化和持久化；切换后立即刷新主状态、连接提示、设置、气泡、动作选择、资源包反馈及完整 MCP 接入向导。Agent 原始事件文本、会话标题和资源包自有名称保持原文。
- 原生窗口每次移动时检查显示器工作区；仅允许透明窗口留白按目标 DPI 在外沿越界最多 36 个逻辑像素，使桌宠贴边但宠物可视主体和控制区不出屏；相邻显示器共享边界时仍可跨屏移动。
- 全套检查通过 25/25 Vitest 文件、118/118 前端测试和 62/62 Rust 测试，TypeScript、Rust check/fmt 与 production build 均通过；收紧边缘策略后 11/11 窗口定向测试再次通过，隔离 Debug EXE 也已从当前源码重建。原生套件在进入本次新断言前，被 Windows 拒绝探针恢复鼠标坐标的 `SetCursorPos` 调用，run `.cache/native-windows-smoke/1785307254272-2780` 仍 `cleanupSafe=true`，因此本轮不声明真实桌面边缘停靠或语言切换已通过原生输入验证。

## 2026-07-29 透明区域鼠标穿透

- Windows 顶层窗口安装有界 `WM_NCHITTEST` 处理：实际交互矩形继续交给 WebView，其余位置返回 `HTTRANSPARENT`，不再让 360×440 的透明窗口整体遮挡下层桌面或应用。
- 前端按当前 DPI 上报宠物、状态栏、可见气泡和错误提示的物理矩形；`ResizeObserver` 与 `hidden` 属性观察负责在缩放、气泡计时和侧栏开关后更新，不使用永久轮询。设置或向导打开时临时捕获整窗，以保留“点击面板外关闭”；关闭后立即恢复紧凑区域。
- 原生拖拽事务从按下到松开期间临时暂停穿透命中，避免短点击误判成拖动；`WM_MOUSELEAVE` 与透明区域命中共同通知前端结束悬停，避免设置按钮或气泡计时残留。当前源码 run `.cache/native-windows-smoke/1785312588253-35044` 完整通过，使用 30,863,872-byte Debug EXE（SHA-256 `AE860EEF5E0BB18BDC62D384651E41F35822CB5896F39A550F934E2D19B81ECB`），拖动/爱心、边缘约束、透明合成、设置、七状态、60 秒 sleeping、托盘及清理均通过。`npm run check` 通过 26/26 Vitest 文件、121/121 前端测试和 64/64 Rust 测试。

## 2026-07-28 拖拽边界与点击爱心

- 原生窗口每次移动时检查所有显示器工作区的并集；当时的实现把超出部分夹回完整可见范围，相邻显示器共享边界时仍可跨屏移动。该策略已被 2026-07-29 的最小可抓取区域策略替代。
- `clicked` 交互保留角色包自定义动作能力，并统一叠加三颗上浮爱心；系统减少动态效果时改为短暂静态爱心，不执行位移动画。
- 真实鼠标输入不依赖 WebView 在原生窗口拖动后补发 `click`：Rust 在启动原生窗口拖动前采样 Windows 指针坐标，轮询左键状态，并在松开后再次采样；300 ms 内真实松开且物理指针位移不超过 5 px 时直接触发爱心。窗口边界修正产生的异步窗口位移不会干扰单击判断；浏览器预览、键盘和辅助技术激活继续走 DOM 点击路径。
- 窗口约束 10/10 Rust 测试、点击控制器 7/7 定向 Vitest、`npm run check:rust`、`npm run check:frontend`、production build 和 native-smoke 脚本语法检查通过。当前源码使用隔离 Cargo 目录完整重建后，run `.cache/native-windows-smoke/1785304253938-33612/desktop-integration.json` 通过真实 Win32 鼠标输入：单击记录 `releasedWithinTimeout=true`、指针位移 `(0,0)`、`recognizedAsClick=true`，进入 `clicked`，三颗爱心的动画名均为 `click-heart-float`，650 ms 后恢复；真实 `(96,64)` 拖动历史没有 `clicked`，向左上外沿拖动后窗口被约束在工作区内。该 run 后续在 Windows 托盘 UI Automation 的 `RPC_E_SERVERFAULT` 环境错误处停止，`cleanupSafe=true`，因此这里只声明本次桌面交互段通过，不声明整套原生冒烟成功。
