...
应用只面向 Windows 发布，优先保证低资源占用、低打扰、本地通信和简单安装。首版复用现有 `furry-ai-state` / `furry-companion-mcp` 的状态协议与本地 IPC 链路，不在桌宠进程内重新实现 MCP stdio transport。
...
5. 仅支持 Windows，不维护 macOS 或 Linux 运行、构建与发布路径。
...
- `session_title`：可选会话标题；桌宠在消息气泡顶部显示，以区分不同会话。
- `message`：当前动作的短说明；当 `state` 为 `success` 时，它承载 Agent 返回给用户的任务结束语。
- `file`：Agent 当前处理的文件路径。

普通详情气泡默认仅在状态变化后或悬停桌宠时短暂显示，用户可以关闭气泡。`file` 字段继续为协议兼容而接收，但产品界面不显示文件路径，也不提供“显示文件”设置。
...
- 新的合法事件立即覆盖当前展示状态。
- 重复状态事件更新 `session_title`、`message` 和 `file`，但不重播入场动画，也不重置 success 返回 idle 或 idle 延迟变体的计时。
...
- 不在任务栏中显示普通应用图标，主要入口位于 Windows 系统托盘。
- 支持鼠标拖动、多显示器和不同 DPI。
...
- 支持 `session_title`、`message` 和 `file` 的可选气泡展示；标题与正文均按纯文本渲染。
...
- Windows 托盘包含显示/隐藏、恢复默认位置、总在最前、设置、重新连接和退出。
- 左键单击显示或隐藏桌宠，右键打开菜单。
...
- 桌宠缩放建议范围 50%–200%。
- 透明度建议范围 30%–100%。
- 总在最前、开机自启和状态气泡开关；不提供文件路径显示开关。
- 手动重新连接；MCP 默认始终启用且地址不可在设置页修改。
- 选择内置角色包或已导入的本地角色包，并为每个状态选择包内动作。每个状态只能显示和选择资源包为该状态声明的候选动作，旧设置中的跨状态动作不得渲染。七种状态各占满一行，点击状态后在该行下方展开候选动作。状态与候选卡片无需悬停即可直接看到由真实媒体首帧生成的静态预览；当前状态或当前选中的候选动作使用唯一的实时动画预览，避免同时解码全部 GIF。
- 内置系统资源包在下拉框中显示为 `Furry AI State`，不附加版本号；导入的本地包仍可显示来源和版本用于区分。
- 从设置页选择 `pet.json` 安全导入本地角色包；导入完成后不依赖源目录，并允许删除应用自己的快照。
- 成功气泡固定展示 15 秒，不提供展示时长设置项。
- 恢复默认设置。
- 设置面板标题栏可直接拖动原生窗口；关闭按钮和表单控件不得触发窗口拖拽。
- 设置面板在桌宠旁侧展开，不得覆盖桌宠，以便调整缩放、透明度和动作时实时观察效果。
- 打开设置或接入向导时根据当前显示器工作区自动选择桌宠左侧或右侧；优先保持整个窗口可见，关闭后桌宠回到打开前的锚点。
- 关闭按钮固定在设置面板右上角，不随设置内容滚动；设置页不提供恢复窗口位置按钮。
- 设置入口位于桌宠底部状态文字旁；默认隐藏，用户悬停桌宠后显示，移向按钮时不得提前消失，键盘聚焦与设置已打开时保持可用。
- 设置打开时，点击设置面板以外的任意区域立即关闭设置，包括宠物本体、状态栏、消息气泡和透明空白；只有设置面板内部交互保持面板打开。

#### F-06 首次接入引导

- 方法 1 提供一段可直接发送给 Codex 或其他可编辑自身 MCP 配置的 Agent 的完整安装提示词，包含 Codex 命令、Cursor / Claude Desktop JSON 配置、重启与 `set_state` 检查，以及 Node.js、包运行和 npm 缓存排障步骤。
- 方法 2 单独展示可手动加入客户端的 `mcpServers.furry_companion` JSON，并明确配置变更后必须重启 Agent；当前会话通常不能热加载新 MCP 工具。
- 完整 Agent 指令以及向导展示的每个命令、JSON 代码段均有独立复制按钮和复制结果反馈。
- 展示连接状态和“重新检测”；界面不显示“最近事件”时间。
- 不自动修改 Agent 配置，除非用户对未来安装功能明确授权。
- 设置页提供“项目介绍”按钮，通过系统默认浏览器打开产品飞书介绍页；外部打开权限只允许该固定 HTTPS URL。
...
- `type` 必须为 `state`。
- `state` 必须属于七种已知状态。
- `session_title`、`message`、`file` 为可选字符串，统一使用 1000 字符上限。
- `furry-companion-mcp` 0.2.0 尚未在 `set_state` 工具中提供 `session_title`；缺少标题时保持 0.2.x 兼容展示。
- 消费端忽略未知字段，保证向前兼容。
...
- 构建发布：Windows GitHub Actions 原生构建。

Tauri 2 复用 Windows WebView2，适合透明小窗口和托盘应用，并提供窗口透明、置顶、跳过任务栏、忽略鼠标事件和保存窗口状态等能力。实际透明窗口、鼠标穿透和桌面层级需要在受支持的 Windows 版本上验收。
...
```text
src-tauri/
  IPC Client          Windows Named Pipe 与增量 JSON Lines 解析
...
内置包已从相邻 `furry-ai-state/media/images` 接入七状态的 22 个逻辑预设，并通过共享 `source` 将重复字节去重为 13 份实际媒体。资源包以“目录索引 + manifest + 动画字典 + 状态映射 + 状态候选映射 + 延迟变体 + 可选交互映射 + 可选静态替代映射”组织：Agent 状态、拖拽等交互和 reduced-motion 只引用动画 ID，不直接写文件路径；每个状态的设置候选由 `stateAnimationChoices` 显式限定。四个初始 GIF 的文件与哈希保持不变，公开分发前仍需补齐作者和美术授权。
...
| 动画与状态候选 | 动画 1–64 个；每状态 1–16 个候选且整包最多 64 个；默认与延迟动作必须属于该状态候选 |
...
2026-07-28 已把相邻 `furry-ai-state/media/images` 的七状态预设接入内置包：22 个逻辑动作通过共享 source 去重为 13 份唯一媒体。manifest、TypeScript loader/运行时和 Rust 本地包验证共同限制每种状态只能选择 `stateAnimationChoices` 中为该状态声明的动作；旧 Store 中的跨状态 override 会自动回退到该状态默认动作。`npm run check` 通过 19/19 Vitest 文件、94/94 前端测试和 57/57 Rust 测试；完整重建的 Windows Debug run `.cache/native-windows-smoke/1785220817686-16956` 为 `success=true`、`cleanupSafe=true`，并在真实 WebView2 中确认 coding 只出现 3 个本状态候选、跨状态 error 动作不可见，选择/恢复与包内 reduced-motion 静态替代均正确。资源作者和美术再分发授权仍须在公开发布前确认。
...
2026-07-16 安装器基线的正式 NSIS（5,664,565 bytes，SHA-256 `EA181EFE9A93B153570AA92D893F351049492E821BED7C5064559163E79DF5DF`）已通过 current-user 安装/卸载；隔离 NSIS（5,665,555 bytes，SHA-256 `E629E932BEB152191A46C98EC73A8F102D49FD5FD98488C1F8B343119F39E7AF`）完成安装、全量 native smoke 和卸载，两者均为 `NotSigned`。机器可读报告 `.cache/windows-installer-smoke/1784201848768-33384/installer-smoke-report.json` 记录 `currentSourceBuild=true`、`success=true`，生命周期 `518.730 s`。安装后的精确 Release EXE 为 14,349,824 bytes，SHA-256 `E6DF0C4422886688C22202B334944F57912FA749199BBBB687C3E589181BAD34`；它在 `.cache/native-windows-smoke/1784202165728-19428` 重复七状态、四 GIF 映射、首次向导/Store v1 设置、透明合成、topmost、client/window 几何、真实 `(96,64)` 拖动和全部真实托盘操作。该 Release 早于交互层及 reduced-motion 静态降级。安装后 Release 尚未重复本地角色包原生 dialog/import/delete/恶意包 UI；物理多显示器拔插、混合 DPI、负坐标布局、Windows 10/ARM64、真实外部 Agent UI、签名、美术许可、初始角色的授权 poster 和资源目标仍未完成。详见 [`SMOKE_TEST_REPORT.md`](SMOKE_TEST_REPORT.md)。
...
| 总在最前 | 必须 |
| 系统托盘 | 必须 |
| 多显示器/DPI | 必须，包含负坐标与混合 DPI |
| 开机自启 | 必须，可在设置中启用或停用 |
| 安装包 | NSIS EXE |
| CPU 架构 | x64 首发，ARM64 需单独验证后再声明支持 |

构建、CI、安装器和发布只维护 Windows 路径。Windows 10、Windows 11、x64 与 ARM64 的支持结论必须分别来自对应环境验证。
...
- 稳态内存 RSS 目标不高于 80 MB，记录 Windows 实际值。
- 冷启动到显示桌宠目标小于 2 秒。
...
1. 在受支持的 Windows 环境安装并启动，能看到透明背景桌宠。
...
6. `session_title`、`message` 和 `file` 按约定显示，恶意 HTML 字符串不会执行；不同标题能区分消息来源会话。
...
11. 使用统一测试脚本记录 Windows CPU、内存、启动耗时和状态延迟。
...
15. 设置入口默认隐藏并位于状态文字旁，悬停桌宠后可见且可稳定点击；打开设置或接入向导时根据工作区空间自动在桌宠左侧或右侧出现，完整窗口保持可见且桌宠不被覆盖；设置关闭按钮在任意滚动位置都保持于右上角。
16. 接入向导同时提供 Agent 自动配置指令和手动 MCP 配置；完整提示词及每个命令/JSON 代码段均可独立复制，设置页“项目介绍”使用系统浏览器打开固定飞书页面。
17. Windows 安装 EXE 使用 FAS Logo，并在交互式安装流程中展示安装目录选择页。
18. 用户可在设置中启用或停用开机自启；设置、Windows 启动注册和重启后的 UI 状态保持一致。
19. 七种状态在动作设置中各占满一行；点击状态后候选动作在该行下方展开。状态与候选动作无需悬停即可看到对应媒体预览，同时页面最多只播放一个实时动画。
20. 每种状态只列出 `stateAnimationChoices` 为该状态声明的预设；跨状态覆盖从 UI、清单验证和运行时解析三层拒绝。
...
- 验证 Windows 10/11、多显示器、负坐标、混合 DPI 与目标 CPU 架构。
- 在 Windows runner 验证 CI，并补齐原生运行与安装冒烟测试。
- 验证开机自启注册、升级、卸载和残留清理。

### M3：正式版准备

- 完整诊断页、角色包导出与管理增强，以及为初始角色补齐获授权的专用静态 poster；开机自启和按系统偏好静态降级能力已完成。
...
| 风险 | 影响 | 应对 |
| --- | --- | --- |
| Windows 多显示器、负坐标和混合 DPI 导致侧栏离屏 | 设置不可见或桌宠位置跳变 | 使用工作区物理坐标选择左右方向并做原生多屏验证 |
| `furry-companion-mcp` 0.2.0 尚未上报会话标题 | 旧 runtime 无法区分气泡会话 | 消费端先兼容可选 `session_title`；推动 runtime 后续暴露该字段 |
...
| 安装包未签名 | 系统安全告警 | 正式发布前配置 Windows 代码签名 |
...
4. 首发仅支持 Windows x64，还是还需同步支持 Windows ARM64？
5. MVP 是否需要点击互动、跟随鼠标、桌面行走，还是仅要求拖动和状态动画？
6. 角色资源偏好 GIF/WebP，还是统一精灵图？
7. 计划开源发布到 GitHub Releases，还是作为内部/个人工具使用？

## 15. 需求冻结前的默认决策

- Tauri 2 + Rust + Vanilla TypeScript。
- 仅开发、测试和发布 Windows 版本。
