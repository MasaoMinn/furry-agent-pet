...
- 支持鼠标拖动、多显示器和不同 DPI。
- 拖动桌宠或设置面板时实时约束原生窗口；允许透明窗口留白在显示器外沿最多越界 36 个逻辑像素，使桌宠贴边停靠，但宠物可视主体和控制区不得被拖出工作区。相邻显示器之间仍允许正常跨屏拖动。
- 保存并恢复窗口位置、缩放比例和显示/隐藏状态。
- 屏幕断开后，桌宠不能永久停留在不可见区域。
- 恢复位置时优先选择最近显示器，并按目标工作区与 DPI 留出安全边距；完全离屏的位置会回到可见工作区，只有透明留白在 36 个逻辑像素限额内越界的边缘停靠位置保持不变。
- 默认不抢夺键盘焦点。
- 设置与向导关闭时，透明窗口不以整个原生矩形捕获鼠标；宠物动作区域按当前图片透明度生成有界命中遮罩，而不是捕获整张图片矩形；状态栏、可见气泡和致命错误提示按其可见矩形参与 Windows 命中测试，其余透明区域必须把点击穿透给桌面或下层应用。设置或向导打开时可临时捕获整窗，以支持点击面板外关闭，关闭后必须立即恢复紧凑捕获范围。
...
- 用户单击桌宠时，由 Windows 原生层在启动窗口拖动前后采样指针位置并等待真实松开，根据按压时限与指针位移识别单击，触发 `clicked` 动作并在角色上方冒出三颗爱心；角色包可继续用 `interactions.clicked` 替换角色自身动作，爱心作为统一点击反馈保留。真实拖动不得误触，键盘激活仍可触发。
...
- 提供中文与 English 界面语言切换；切换后立即更新应用自有文案、状态名、气泡、接入向导和动态反馈，并通过现有设置 Store 在重启后恢复。Agent 原始消息、会话标题、资源包名称和技术错误详情保持原文。
...
2. 用户可拖动、缩放、隐藏桌宠；可将桌宠停靠到显示器外沿，只有透明留白可按 DPI 越界最多 36 个逻辑像素，宠物可视主体和控制区保持在工作区内，重启后恢复位置和设置。
...
8. 单击桌宠触发有界 `clicked` 动作并显示爱心，真实拖拽结束不会误触该反馈。
9. 关闭并重启 MCP Runtime，桌宠不崩溃并自动重连。
10. 发送非法 JSON、未知状态和超长字段，应用不崩溃且诊断可定位问题。
11. 托盘菜单能完成显示/隐藏、重连、恢复位置、设置和退出。
12. 使用统一测试脚本记录 Windows CPU、内存、启动耗时和状态延迟。
13. 导入有效本地包后，移动或删除源目录并重启应用仍能显示快照；路径穿越、媒体伪装和超限资源被拒绝；删除本地包不会影响受管目录之外的文件。
14. 从已断开的显示器恢复位置时，桌宠回到当前显示器工作区；负坐标布局和不同 DPI 下不会永久离屏，仍部分可见的位置不会被擅自移动。
15. `error` 气泡不会自动关闭，可由后续合法状态覆盖或由用户确认；确认只改变本地展示，不向 MCP Runtime 回写事件。
16. 设置入口默认隐藏并位于状态文字旁，悬停桌宠后可见且可稳定点击；打开设置或接入向导时根据工作区空间自动在桌宠左侧或右侧出现，完整窗口保持可见且桌宠不被覆盖；设置关闭按钮在任意滚动位置都保持于右上角。
17. 接入向导同时提供 Agent 自动配置指令和手动 MCP 配置；完整提示词及每个命令/JSON 代码段均可独立复制，设置页“项目介绍”使用系统浏览器打开固定飞书页面。
18. Windows 安装 EXE 使用 FAS Logo，并在交互式安装流程中展示安装目录选择页。
19. 用户可在设置中启用或停用开机自启；设置、Windows 启动注册和重启后的 UI 状态保持一致。
20. 七种状态在动作设置中各占满一行；点击状态后候选动作在该行下方展开。状态与候选动作无需悬停即可看到对应媒体预览，同时页面最多只播放一个实时动画。
21. 每种状态只列出 `stateAnimationChoices` 为该状态声明的预设；跨状态覆盖从 UI、清单验证和运行时解析三层拒绝。
22. 用户可在设置中切换中文与 English；主界面、设置、接入向导、状态名和应用生成的气泡/反馈立即切换，重启后保持所选语言。
23. 宠物周围以及宠物图片内部的透明区域不拦截鼠标，用户可正常点击其下方的桌面图标或应用控件；宠物可见像素附近保留小幅点击余量，状态栏、可见气泡和侧栏仍可正常点击与拖动。
